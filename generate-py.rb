...
def quote(re)
  re.inspect[1...-1]  # escape Unicode and nix .inspect's quotes
    .gsub(/[\x00-\x19]/) { |x| "\\x#{x.ord.to_s(16).rjust(2, '0')}" }  # control chars
    .gsub(/[\u0080-\uffff]/) { |x| "\\u#{x.ord.to_s(16).rjust(4, '0')}" }  # non-ascii BMP
end
