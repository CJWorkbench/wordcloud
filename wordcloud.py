...
def text_to_tokens(text: str) -> Iterable[bytes]:
    ...
    """
    # Slow imports and regex-compilations are here so they don't get
    # invoked during migrate_params().
    import nltk.corpus
    import re2
    ...
    STOPWORDS = set(
        word.encode("utf-8") for word in nltk.corpus.stopwords.words("english")
    )
...
def most_common_tokens(
    tokens: Iterable[bytes], max_n_tokens=100
) -> List[Tuple[str, int]]:
    """
